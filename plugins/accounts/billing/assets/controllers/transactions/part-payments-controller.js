...
.controller("accountsBillingTransactionsPartPaymentsController", function($scope, utils, patientEventLogger){
		...
		managePaymentRequest:{
			...
			viewPaymentBill: function(id){
				$scope.temp = {
					...
					transactionStatus: $(".btn-payment-request[data-option-id='"+id+"']").attr("data-option-transaction-status")
				};
	...
	.withFnServerData(function(source, data, callback, settings){
		...
		if (filter.type == 'date'){
			...
			_filter += 'filtertype=paymentstatus&query='+filter.value;
		}
	...
	$scope.dtColumns = [
		utils.DT.columnBuilder.newColumn(null).withTitle("Request Details").renderWith(function(data, full, meta){
			if (typeof data.AttachedInvoiceNumber !== "undefined" && data.AttachedInvoiceNumber != null){
