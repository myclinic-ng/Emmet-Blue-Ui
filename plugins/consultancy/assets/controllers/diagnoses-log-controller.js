...
.controller('consultancyDiagnosesLogController', function($scope, utils, patientEventLogger, $rootScope){
	...
	.withFnServerData(function(source, data, callback, settings){
        ...
        var id = utils.userSession.getID();
        var url = '/consultancy/logs/get-diagnoses?resourceId='+id+'&'+_filter+'&paginate&from='+start+'&size='+length;
	...
	$scope.dtColumns = [
		utils.DT.columnBuilder.newColumn(null).withTitle("ID").notSortable().renderWith(function(data, full, meta){
			var html="<span class='text-small'>#"+data.DiagnosisID+"</span>";
		...
		utils.DT.columnBuilder.newColumn(null).withTitle("Diagnosis").notSortable().renderWith(function(data, full, meta){
			var html=data.DiagnosisTitle;
			
			return html;
		}),
		utils.DT.columnBuilder.newColumn(null).withTitle("Time").renderWith(function(data, a, b){
			var time = '<b>'+(new Date(data.DateLogged)).toDateString()+", "+(new Date(data.DateLogged)).toLocaleTimeString();
